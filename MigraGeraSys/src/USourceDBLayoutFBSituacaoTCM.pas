unit USourceDBLayoutFBSituacaoTCM;
...
uses
  ...
  cxGridDBTableView, cxGrid, cxDBLookupComboBox, Vcl.Samples.Gauges, cxTextEdit;

type
  TfrmSourceDBLayoutFBSituacaoTCM = class(TfrmPadraoSDI)
  ...
  procedure ShowParametrizarVinculos(const AOnwer : TComponent;
...
procedure ShowParametrizarVinculos(const AOnwer : TComponent;
  const aCompetencia : TGenerico; const aTabela, aCampoReferencia, aDescricao : String);
var
  aForm : TfrmSourceDBLayoutFBSituacaoTCM;
begin
  aForm := TfrmSourceDBLayoutFBSituacaoTCM.Create(AOnwer);
  try
    with aForm do
    begin
...
function TfrmSourceDBLayoutFBSituacaoTCM.ConfirmarProcesso: Boolean;
var
  I : Integer;
  aPosicao   ,
  aCodigo    ,
...
procedure TfrmSourceDBLayoutFBSituacaoTCM.FormCreate(Sender: TObject);
begin
  inherited;
...
initialization
  gFormularios.RegisterForm('frmSourceDBLayoutFBSituacaoTCM', TfrmSourceDBLayoutFBSituacaoTCM);
